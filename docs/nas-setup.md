...
- **The page is `noindex`, but crawling is ALLOWED.** That combination is deliberate and the ordering matters: `robots.txt` controls crawling, the meta tag controls indexing, and blocking the crawl stops Google from ever reading the `noindex` — which is how a bare, snippet-less URL gets stuck in the index. `public/robots.txt` carries the full explanation and the launch-day checklist.
- **Webroot is `public/`.** Everything public lives there and the Dockerfile copies the whole directory. Do not go back to listing filenames in `COPY` — that ships builds which succeed, deploy, pass the health check, and 404 on the new asset. Note also that `.dockerignore` excludes `assets/` and `docs/`, so a `COPY` from either hard-fails.
